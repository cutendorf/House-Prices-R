...
result[3,2]<-as.numeric(rf$r.squared)
```
...
rf_residuals <- df_test$price - df_test$test_rf

residual_plot<-as.data.frame(df_test$price)
residual_plot<-cbind(residual_plot,rf_residuals)

ggplot(residual_plot, aes(x=df_test$price, rf_residuals)) +
    geom_point(shape=1) +
    geom_smooth()+
    theme_minimal()+
    ylab('Residuals')+
...
result[3,3]<-as.numeric(rmse(real=whole_data$holdout$price, predicted = test_rf))
result[3,4]<-as.numeric(mae(real=whole_data$holdout$price, predicted = test_rf))
result[3,5]<-as.numeric(mape(real=whole_data$holdout$price, predicted = test_rf))
datatable(result)
```
...
* month: month in which house was sold - there might be seasonal effects

Due to the fact that month of sale is extracted the date column is not needed anymore and thus excluded from the data set.
...
df <- df[ , !(names(df) %in% c('id', 'month', 'zipcode', 'lat', 'long'))]
...
As we can see the log of prices is actually quiet close to a normal distribution. It might make sense to actually use the log of the target for the model and then transform back the predictions on the initial scale.
...
```{r}
discVar <- c("bedrooms", "bathrooms", "floors", "waterfront", "view", "condition",  "renovated", "basement", "grade", "month")
...
Similar to the discrete variables, we also have a look at the several continous variables. Six of them are all related to squarefeet measures, the the others are houseage and grade . In order to plot them properly a new melted dateframe is created.
...
corr_data <- corr_data[ , !(names(corr_data) %in% c('id', 'zipcode'))]
...
Also for the continous variables we that that most of them show some kind of linear relation ship to the target. However, the variables sqft_lot, sqft_lot15 and house age do not show a good relationship and very low correlation values to the target. Thus it might make sense to exclude them from the model since their correlation is less than 0.1. 
...
#### 1.5.2 Low correlated features
As we have seen before in the correlation analysis there are several features that should be excluded due to low correlation with the target:

* condition: corr = 0.04
* sqft_lot15 : cor = 0.08

*sqft_lot will be included for now eventhough corr = 0.09 to test significance in model*

* houseage: corr = -0.05

*yr_built will be included for eventhough corr = 0.05 to test significance in model and to create a new variable based on decades*

* month: corr = 0.01

A function was created to delete these columns from train and test data. No threshold for the correlation is set here but the columns are called by there names since as described above some of the low correlation features might be used in the next step.
```{r}
prep_train_data2 <- low_correlation(prep_train_data)
prep_test_data2 <- low_correlation(prep_test_data)
```
...
```{r, warning=FALSE, message=FALSE}
prep_train_data3 <- data_types(prep_train_data2)
prep_test_data3 <- data_types(prep_test_data2)
...
##### Dummy encode
```{r}
total_encoded<-caret::dummyVars(formula= ~.- id , data = total_data, fullRank=F,sep = "_")

...
Furthermore, the variable condition shows a very low correlation of only 0.04 and thus since its below 0.05 should be excluded for the modeling.
...
##### Dummy encode
```{r}
total_encoded<-caret::dummyVars(formula= ~.- id - date, data = total_data, fullRank=T,sep = "_")
...
At the end the data sets are split again based on the classification before on train and test. The train dataset has still 17,276 rows (one row deleted due to too many bedrooms) and the test 4,320 rows.
...
fwrite(data_train_ready, 'data/data_train_ready.csv', row.names = F)
fwrite(data_test_ready, 'data/data_test_ready.csv', row.names = F)
```